...
class SyncCJit extends BaseCJit
{
    ...
    execGyp(codePath,command)
    {
        ...
        return child_process.execSync(`cd ${codePath.replace(' ','\\ ')} && ${this.gypBinPath.replace(' ','\\ ')} ${command}`);
    }
    ...
    delete(codeMd5,codePath)
    {
        ...
        for(let filename of files)
        {
            ...
            {
                this.delete(codeMd5,filePath);
            } else {
                fs.unlinkSync(filePath);
            }
        }
        fs.rmdirSync(codePath);
    }
}
